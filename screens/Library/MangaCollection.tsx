import { FullManga, LibraryStackParamsList } from "../../common/types";
...
import { MangaGridList } from "../../features/library";
import { getCollection } from "../../lib";
import { useStore } from "../../store";
import Toast from "react-native-toast-message";
import { Loader, CollectionLayout } from "../../components";

export const MangaCollection: React.FC<
  StackScreenProps<LibraryStackParamsList, "MangaCollection">
> = ({ route }) => {
  ...
  const [data, setData] = useState<FullManga[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isRefreshing, setIsRefreshing] = useState(false);

  const fetchCollection = useCallback(async () => {
    try {
      const { error, data } = await getCollection(collection.key, user.id, false);

      if (error) {
        throw new Error("Unable to fetch collection.");
      }
      if (data) {
        setData(data as FullManga[]);
      }
    } catch (error) {
  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <MangaGridList data={data} isRefreshing={isRefreshing} refresh={refresh} />
      )}
      {filters && (
        <FiltersSheet
          query={query}
